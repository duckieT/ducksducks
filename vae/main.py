...
train_loader = torch.utils.data.DataLoader(
    datasets.ImageFolder('./imgs', transform=transforms.ToTensor()),
...
class VAE(nn.Module):
    ...
    def forward(self, x):
        mu, logvar = self.encode(x.view(-1, dimesions[0] * dimesions[1]))
...
def loss_function(recon_x, x, mu, logvar):
    BCE = F.binary_cross_entropy(recon_x, x.view(-1, dimesions[0] * dimesions[1]), reduction='sum')
